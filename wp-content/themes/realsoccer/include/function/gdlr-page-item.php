	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if( empty($atts['title-type']) && !empty($atts['type']) ){
				$atts['title-type'] = $atts['type'];
			}
			...
			if( !empty($atts['title-type']) && $atts['title-type'] != 'none' ){
				...
				if(!empty($atts['carousel'])){
					...
					$ret .= '<i class="icon-angle-left fa fa-angle-left gdlr-flex-prev"></i>';
					$ret .= '<i class="icon-angle-right fa fa-angle-right gdlr-flex-next"></i>';
					$ret .= '</div>';
				}
