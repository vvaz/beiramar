	...
	if( !function_exists('gdlr_get_blog_info') ){
		function gdlr_get_blog_info( $array = array(), $wrapper = true, $sep = '' ){
			...
			foreach($array as $post_info){
				...
				switch( $post_info ){
					case 'date':
						$ret .= '<div class="blog-info blog-date post-date updated"><i class="icon-calendar fa fa-calendar"></i>';
					...
					case 'author':
						...
						$ret .= '<div class="blog-info blog-author vcard author post-author"><i class="icon-user fa fa-user"></i>';
						$ret .= '<span class="fn">'.$author.'</span>';
						$ret .= '</div>';						
						break;						
				}
			}
