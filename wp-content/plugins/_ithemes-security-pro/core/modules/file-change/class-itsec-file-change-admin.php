...
class ITSEC_File_Change_Admin {
		...
		if ( isset( get_current_screen()->id ) && ( false !== strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_settings' ) || false !== strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_logs' ) || false !== strpos( get_current_screen()->id, 'dashboard' ) ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'button_text'          => isset( $this->settings['split'] ) && true === $this->settings['split'] ? __( 'Scan Next File Chunk', 'it-l10n-ithemes-security-pro' ) : __( 'Scan Files Now', 'it-l10n-ithemes-security-pro' ),
	...
	public function file_change_form( $origin ) {
		...
		echo '<p><input type="submit" id="itsec_one_time_file_check_submit" class="button-primary" value="' . ( isset( $this->settings['split'] ) && true === $this->settings['split'] ? __( 'Scan Next File Chunk', 'it-l10n-ithemes-security-pro' ) : __( 'Scan Files Now', 'it-l10n-ithemes-security-pro' ) ) . '" /></p>';
	...
	public function metabox_advanced_file_change_settings() {
		...
		if ( ITSEC_Lib::get_memory_limit() < 128 ) {
			echo '<div class="itsec-warning-message">' . __( 'Warning: Your server has less than 128MB of RAM dedicated to PHP. If you have many files in your installation or a lot of active plugins activating this feature may result in your site becoming disabled with a memory error. See the plugin homepage for more information.', 'it-l10n-ithemes-security-pro' ) . '</div>';
		}
	...
	public function settings_field_email() {
		...
		echo '<label for="itsec_file_change_email"> ' . __( 'Email file change notifications', 'it-l10n-ithemes-security-pro' ) . '</label>';
		echo '<p class="description">' . __( 'Notifications will be sent to all emails set to receive notifications on the global settings page.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function settings_field_enabled() {
		...
		echo '<label for="itsec_file_change_enabled"> ' . __( 'Enable File Change detection', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function settings_field_file_list() {
		...
		echo '<p class="description">' . __( 'Exclude files or folders by clicking the red minus next to the file or folder name.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function settings_field_notify_admin() {
		...
		echo '<label for="itsec_file_change_notify_admin"> ' . __( 'Display file change admin warning', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function settings_field_types() {
		...
		echo '<label for="itsec_file_change_types"> ' . __( 'File types listed here will not be checked for changes. While it is possible to change files such as images it is quite rare and nearly all known WordPress attacks exploit php, js and other text files.', 'it-l10n-ithemes-security-pro' ) . '</label>';
