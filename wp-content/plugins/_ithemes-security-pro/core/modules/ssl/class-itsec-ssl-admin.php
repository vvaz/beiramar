...
class ITSEC_SSL_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$id    = 'ssl_options';
		$title = __( 'Secure Socket Layers (SSL)', 'it-l10n-ithemes-security-pro' );
	...
	public function admin_script() {
		...
		if ( isset( get_current_screen()->id ) && strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_settings' ) !== false ) {
			...
			wp_localize_script( 'itsec_ssl_js', 'ssl_warning_text', array( 'text' => __( 'Are you sure you want to enable SSL? If your server does not support SSL you will be locked out of your WordPress Dashboard.', 'it-l10n-ithemes-security-pro' ) ) );
	...
	public function initialize_admin() {
		...
		add_settings_section(
			'ssl_settings',
			__( 'Configure SSL', 'it-l10n-ithemes-security-pro' ),
	...
	public function metabox_advanced_settings() {
		...
		if ( $this->has_ssl === false ) {
			...
			$content .= sprintf( '<div class="itsec-warning-message"><span>%s: </span>%s</div>', __( 'WARNING', 'it-l10n-ithemes-security-pro' ), __( 'Your server does not appear to support SSL. Your server MUST support SSL to use these features. Using these features without SSL support on your server or host will cause some or all of your site to become unavailable.', 'it-l10n-ithemes-security-pro' ) );

		} else {

			$content .= sprintf( '<div class="itsec-notice-message"><span>%s: </span>%s</div>', __( 'WARNING', 'it-l10n-ithemes-security-pro' ), __( 'Your server does appear to support SSL. Using these features without SSL support on your server or host will cause some or all of your site to become unavailable.', 'it-l10n-ithemes-security-pro' ) );

		}

		$content .= '<p>' . __( 'Note: When turning SSL on you will be logged out and you will have to log back in. This is to prevent possible cookie conflicts that could make it more difficult to get in otherwise.', 'it-l10n-ithemes-security-pro' ) . '</p>';
