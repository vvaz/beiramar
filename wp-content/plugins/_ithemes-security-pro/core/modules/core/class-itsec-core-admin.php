...
class ITSEC_Core_Admin {
	...
	public function add_admin_meta_boxes( $available_pages ) {
		...
		foreach ( $available_pages as $page ) {
			...
			add_meta_box(
				...
				__( 'Download Our WordPress Security Pocket Guide', 'it-l10n-ithemes-security-pro' ),
			...
			add_meta_box(
				...
				__( 'Manage Your Sites Remotely', 'it-l10n-ithemes-security-pro' ),
	...
	public function add_plugin_meta_links( $meta ) {
		...
		$meta[] = '<a href="https://ithemes.com/security?utm_source=wordpressadmin&utm_medium=banner&utm_campaign=itsecfreecta" target="_blank">' . __( 'Get Support', 'it-l10n-ithemes-security-pro' ) . '</a>';
	...
	public function metabox_get_started() {
		...
		echo sprintf( '<p class="itsec-video-description">%s <a href="http://ithem.es/6y" target="_blank">%s</a> %s </p>', __( 'In this short video, we walk through', 'it-l10n-ithemes-security-pro' ), __( 'how to get started securing your site', 'it-l10n-ithemes-security-pro' ), __( 'with iThemes Security.', 'it-l10n-ithemes-security-pro' ) );
		...
		echo '<h2>' . __( 'Website Security is a complicated subject, but we have experts that can help.', 'it-l10n-ithemes-security-pro' ) . '</h2>';
		echo '<p>' . __( 'Get added peace of mind with professional support from our expert team and pro features to take your site security to the next level with iThemes Security Pro.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		echo '<p><a class="button-primary" href="https://ithemes.com/security?utm_source=wordpressadmin&utm_medium=banner&utm_campaign=itsecfreecta" target="_blank">' . __( 'Get Support and Pro Features', 'it-l10n-ithemes-security-pro' ) . '</a></p>';
		echo '</div>';
		echo '</div>';
	...
	public function metabox_need_help() {
		...
		echo '<p><a class="button-secondary" href="https://ithemes.com/security/?utm_source=wordpressadmin&utm_medium=widget&utm_campaign=itsecfreecta" target="_blank">' . __( 'Get iThemes Security Pro', 'it-l10n-ithemes-security-pro' ) . '</a></p>';
	...
	public function metabox_sync_integration() {
		...
		echo '<p><a class="button-primary" href="http://www.ithemes.com/sync" target="_blank">' . __( 'Try iThemes Sync for Free', 'it-l10n-ithemes-security-pro' ) . '</a></p>';
		echo '</div>';
			...
			if ( ! function_exists( 'ithemes_plugin_support_notice' ) ) {
				...
				function ithemes_plugin_support_notice() {
					...
					echo '<div class="updated" id="itsec_support_notice">
						<span class="itsec_notice_text">' . __( 'It looks like you\'ve been enjoying', 'it-l10n-ithemes-security-pro' ) . ' ' . $itsec_globals['plugin_name'] . ' ' . __( "for at least 30 days. It's time to take the next step.", 'it-l10n-ithemes-security-pro' ) . '</span><input type="button" class="itsec-notice-button" value="' . __( 'Upgrade to Pro', 'it-l10n-ithemes-security-pro' ) . '" onclick="document.location.href=\'?itsec_donate=yes&_wpnonce=' . wp_create_nonce( 'itsec-nag' ) . '\';">  <input type="button" class="itsec-notice-button" value="' . __( 'Rate it 5★\'s', 'it-l10n-ithemes-security-pro' ) . '" onclick="document.location.href=\'?itsec_rate=yes&_wpnonce=' . wp_create_nonce( 'itsec-nag' ) . '\';">  <input type="button" class="itsec-notice-button" value="' . __( 'Tell Your Followers', 'it-l10n-ithemes-security-pro' ) . '" onclick="document.location.href=\'?itsec_tweet=yes&_wpnonce=' . wp_create_nonce( 'itsec-nag' ) . '\';">  <input type="button" class="itsec-notice-hide" value="&times;" onclick="document.location.href=\'?itsec_no_nag=off&_wpnonce=' . wp_create_nonce( 'itsec-nag' ) . '\';">
						</div>';
