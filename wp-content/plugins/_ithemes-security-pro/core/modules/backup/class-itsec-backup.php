...
class ITSEC_Backup {
	...
	public function do_backup( $one_time = false ) {
		...
		if ( $itsec_files->get_file_lock( 'backup' ) ) {
			...
			if ( true === $one_time ) {
				...
				$type    = 'updated';
				$message = __( 'Backup Completed and ' . $details, 'it-l10n-ithemes-security-pro' );
			...
			if ( true === $one_time ) {
				...
				$type    = 'error';
				$message = __( 'Something went wrong with your backup. It looks like another process might already be trying to backup your database. Please try again in a few minutes. If the problem persists please contact support.', 'it-l10n-ithemes-security-pro' );
	...
	private function execute_backup( $one_time = false ) {
		...
		if ( 2 !== $this->settings['method'] || true === $one_time ) {
			...
			$body       = __( 'Attached is the backup file for the database powering', 'it-l10n-ithemes-security-pro' ) . ' ' . get_option( 'siteurl' ) . __( ' taken', 'it-l10n-ithemes-security-pro' ) . ' ' . date( 'l, F jS, Y \a\\t g:i a', $itsec_globals['current_time'] );

			//Setup the remainder of the email
			$recipients   = $option['backup_email'];
			$subject      = __( 'Site Database Backup', 'it-l10n-ithemes-security-pro' ) . ' ' . date( 'l, F jS, Y \a\\t g:i a', $itsec_globals['current_time'] );
