...
class ITSEC_Backup_Admin {
	...
	public function admin_enqueue_scripts() {
		...
		if ( isset( get_current_screen()->id ) && strpos( get_current_screen()->id, 'security_page_toplevel_page_itsec_settings' ) !== false ) {
			...
			wp_localize_script( 'itsec_backup_js', 'exclude_text', array(
				'available' => __( 'Tables for Backup', 'it-l10n-ithemes-security-pro' ),
				'excluded'  => __( 'Excluded Tables', 'it-l10n-ithemes-security-pro' ),
	...
	public function metabox_backupbuddy() {
		...
		echo '<h4>' . __( '5 Reasons You Need a Complete Backup Strategy', 'it-l10n-ithemes-security-pro' ) . '</h4>';
		echo '<ol>';
		echo '<li><strong>' . __( 'Database backups aren\'t enough.', 'it-l10n-ithemes-security-pro' ) . '</strong> ' . __( 'You need complete backups of your entire site (including images and media files, themes, plugins, widgets and settings).', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li><strong>' . __( 'Backup files should be protected.', 'it-l10n-ithemes-security-pro' ) . '</strong> ' . __( 'Send and store them safely off-site to a secure remote destination (like email, Dropbox, Amazon S3, etc.)', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li><strong>' . __( 'Backups should be automated and scheduled so you don\'t forget.', 'it-l10n-ithemes-security-pro' ) . '</strong> ' . __( 'Set daily, weekly or monthly backups that automatically send backups off-site.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li><strong>' . __( 'Restoring your site should be quick and easy.', 'it-l10n-ithemes-security-pro' ) . '</strong> ' . __( 'If you get hacked or your server crashes, you shouldn\'t have to worry about restoring your site. Reliable backups mean nothing gets corrupted or broken during the restore process.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li><strong>' . __( 'You should own your backup files.', 'it-l10n-ithemes-security-pro' ) . '</strong> ' . __( 'Don\'t just rely on a host or service. It\'s your site, so you should own everything on it.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '</ol>';

		echo '<p class="bub-cta"><a href="http://ithemes.com/better-backups" target="_blank" class="button-primary" >' . __( 'Learn more about BackupBuddy', 'it-l10n-ithemes-security-pro' ) . '</a></p>';
	...
	public function metabox_one_time() {
		...
		echo '<p>' . __( 'Press the button below to create a backup of your WordPress database. If you have "Send Backups By Email" selected in automated backups you will receive an email containing the backup file.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		echo '<p class="submit"><input type="submit" class="button-primary" value="' . __( 'Create Database Backup', 'it-l10n-ithemes-security-pro' ) . '" /></p>';
		echo '<p><a href="?page=toplevel_page_itsec_settings#itsec_backup_all_sites">' . __( 'Adjust Backup Settings', 'it-l10n-ithemes-security-pro' ) . '</a>';
		echo '</form>';
	...
	public function settings_field_all_sites() {
		...
		echo '<label for="itsec_backup_all_sites"> ' . __( 'Checking this box will have the backup script backup all tables in your database, even if they are not part of this WordPress site.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function settings_field_enabled() {
		...
		echo '<label for="itsec_backup_enabled"> ' . __( 'Enable Scheduled Database Backups', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function settings_field_exclude() {
		...
		echo '<label for="itsec_backup_exclude"> ' . __( 'Tables with data that does not need to be backed up', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function settings_field_interval() {
		...
		echo '<label for="itsec_backup_interval"> ' . __( 'Days', 'it-l10n-ithemes-security-pro' ) . '</label>';
		echo '<p class="description"> ' . __( 'The number of days between database backups.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function settings_field_retain() {
		...
		echo '<p class="description"> ' . __( 'Limit the number of backups stored locally (on this server). Any older backups beyond this number will be removed. Setting to "0" will retain all backups.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function settings_field_zip() {
		...
		echo '<label for="itsec_backup_zip"> ' . __( 'Zip Database Backups', 'it-l10n-ithemes-security-pro' ) . '</label>';
		echo '<p class="description">' . __( 'You may need to turn this off if you are having problems with backups.', 'it-l10n-ithemes-security-pro' ) . '</p>';
