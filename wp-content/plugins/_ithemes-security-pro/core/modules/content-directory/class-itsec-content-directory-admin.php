...
class ITSEC_Content_Directory_Admin {
	...
	protected function undo_change_content_directory() {
		if ( ! wp_verify_nonce( $_POST['wp_nonce'], 'itsec-undo-change-content-directory' ) ) {
			$this->show_error( __( 'Unable to undo the change to the content directory due to a failed nonce verification.', 'it-l10n-ithemes-security-pro' ) );
	...
	protected function show_redirect_message() {
		...
		if ( 'undo-success' === $name ) {
			echo '<div class="updated fade"><p><strong>' . __( 'The Content Directory change was successfully changed back to <code>wp-content</code>.', 'it-l10n-ithemes-security-pro' ) . "</strong></p></div>\n";
		} else if ( 'change-success' === $name ) {
			echo '<div class="updated fade"><p><strong>' . sprintf( __( 'The Content Directory was successfully changed to <code>%s</code>.', 'it-l10n-ithemes-security-pro' ), $arg ) . "</strong></p></div>\n";
		}
	}
	...
	public function metabox_advanced_settings() {
		...
		if ( $this->is_custom_directory() || $this->is_modified_by_it_security() ) {
			...
			echo '<p>' . sprintf( __( 'The <code>wp-content</code> directory is available at <code>%s</code>.', 'it-l10n-ithemes-security-pro' ), $dir_name ) . '</p>';
...
?>
	<form method="post" action="?page=toplevel_page_itsec_advanced&settings-updated=true" class="itsec-form">
		...
		<div class="itsec-warning-message"><?php printf( __( '<span>IMPORTANT:</span> Ensure that you <a href="%s">create a database backup</a> before undoing the Content Directory change.', 'it-l10n-ithemes-security-pro' ), admin_url( 'admin.php?page=toplevel_page_itsec_backups' ) ); ?></div>
		...
		<p class="submit">
			<input type="submit" class="button-primary" name="undo_change_content_directory" value="<?php _e( 'Undo Content Directory Change', 'it-l10n-ithemes-security-pro' ); ?>" />
		</p>
	</form>
<?php
				
			} else {
				echo '<p>' . __( 'No further actions are available on this page.', 'it-l10n-ithemes-security-pro' ) . '</p>';
			}
		} else {
			...
			echo '<div class="itsec-warning-message">' . __( '<span>IMPORTANT:</span> Deactivating or uninstalling this plugin will not revert the changes made by this feature.', 'it-l10n-ithemes-security-pro' ) . '</div>';
			echo '<div class="itsec-warning-message">' . sprintf( __( '<span>IMPORTANT:</span> Ensure that you <a href="%s">create a database backup</a> before changing the Content Directory.', 'it-l10n-ithemes-security-pro' ), admin_url( 'admin.php?page=toplevel_page_itsec_backups' ) ) . '</div>';
			echo '<div class="itsec-warning-message">' . __( '<span>WARNING:</span> Changing the name of the Content Directory on a site that already has images and other content referencing it <strong>will break your site</strong>. For this reason, we highly recommend only changing the Content Directory on a fresh WordPress install.', 'it-l10n-ithemes-security-pro' ) . '</div>';
			...
			if ( false !== apply_filters( 'itsec_filter_can_write_to_files', false ) ) {
				...
				<form method="post" action="?page=toplevel_page_itsec_advanced&settings-updated=true" class="itsec-form">
					...
					<table class="form-table">
						<tr valign="top">
							<th scope="row" class="settinglabel">
								<label for="itsec_enable_content_dir"><?php _e( 'Enable Change Directory Name', 'it-l10n-ithemes-security-pro' ); ?></label>
							</th>
						...
						<tr valign="top" id="content_directory_name_field">
							<th scope="row" class="settinglabel">
								<label for="itsec_content_name"><?php _e( 'Directory Name', 'it-l10n-ithemes-security-pro' ); ?></label>
							</th>
							<td class="settingfield">
					...
					<p class="submit">
						<input type="submit" class="button-primary" value="<?php _e( 'Change Content Directory', 'it-l10n-ithemes-security-pro' ); ?>"/>
					</p>
				</form>
				...
				echo '<p>' . sprintf( __( 'You must allow this plugin to write to the wp-config.php file on the <a href="%s">Settings</a> page to use this feature.', 'it-l10n-ithemes-security-pro' ), admin_url( 'admin.php?page=toplevel_page_itsec_settings' ) ) . '</p>';
			}
		}
	}
	...
	public function process_directory() {
		if ( ! wp_verify_nonce( $_POST['wp_nonce'], 'ITSEC_admin_save' ) ) {
			$this->show_error( __( 'Unable to change the Content Directory due to a failed nonce verification.', 'it-l10n-ithemes-security-pro' ) );
			$this->show_network_admin_notice();
		...
		if ( $this->is_custom_directory() ) {
			$this->show_error( __( 'The <code>wp-content</code> directory has already been renamed. No Directory Name changes have been made.', 'it-l10n-ithemes-security-pro' ) );
			$this->show_network_admin_notice();
		...
		if ( empty( $dir_name ) ) {
			$this->show_error( __( 'The Directory Name cannot be empty.', 'it-l10n-ithemes-security-pro' ) );
			$this->show_network_admin_notice();
		...
		if ( 'wp-content' === $dir_name ) {
			$this->show_error( __( 'You have not chosen a new name for wp-content. Nothing was saved.', 'it-l10n-ithemes-security-pro' ) );
			$this->show_network_admin_notice();
		...
		if ( preg_match( '{^(?:/|\\|[a-z]:)}i', $dir_name ) ) {
			$this->show_error( sprintf( __( 'The Directory Name cannot be an absolute path. Please supply a path that is relative to <code>ABSPATH</code> (<code>%s</code>).', 'it-l10n-ithemes-security-pro' ), ABSPATH ) );
			$this->show_network_admin_notice();
	...
	protected function change_content_directory( $dir_name ) {
		...
		if ( file_exists( $new_dir ) ) {
			if ( $undo ) {
				$this->show_error( sprintf( __( 'A file or directory already exists at <code>%s</code>. The Content Directory change has not been undone. Please remove the existing file or directory and try again.', 'it-l10n-ithemes-security-pro' ), $new_dir ) );
			} else {
				$this->show_error( sprintf( __( 'A file or directory already exists at <code>%s</code>. No Directory Name changes have been made. Please choose a new Directory Name or remove the existing file or directory and try again.', 'it-l10n-ithemes-security-pro' ), $new_dir ) );
			}
		...
		if ( is_wp_error( $modification_result ) ) {
			...
			if ( $rename_result ) {
				...
				$this->show_error( sprintf( __( 'Unable to update the <code>wp-config.php</code> file. No directory or config file changes have been made. %1$s (%2$s)', 'it-l10n-ithemes-security-pro' ), $modification_result->get_error_message(), $modification_result->get_error_code() ) );
				
				$this->show_error( sprintf( __( 'In order to change the content directory on your server, you will have to manually change the configuration and rename the directory. Details can be found <a href="%s">here</a>.', 'it-l10n-ithemes-security-pro' ), 'https://codex.wordpress.org/Editing_wp-config.php#Moving_wp-content_folder' ) );
			} else {
				$this->show_error( sprintf( __( 'CRITICAL ERROR: The <code>%1$s</code> directory was successfully renamed to the new name (<code>%2$s</code>). However, an error occurred when updating the <code>wp-config.php</code> file to configure WordPress to use the new content directory. iThemes Security attempted to rename the directory back to its original name, but an unknown error prevented the rename from working as expected. In order for your site to function properly, you will either need to manually rename the <code>%2$s</code> directory back to <code>%1$s</code> or manually update the <code>wp-config.php</code> file with the necessary modifications. Instructions for making this modification can be found <a href="%3$s">here</a>.', 'it-l10n-ithemes-security-pro' ), $old_name, $new_name, 'https://codex.wordpress.org/Editing_wp-config.php#Moving_wp-content_folder' ) );
				
				$this->show_error( sprintf( __( 'Details on the error that prevented the <code>wp-config.php</code> file from updating is as follows: %1$s (%2$s)', 'it-l10n-ithemes-security-pro' ), $modification_result->get_error_message(), $modification_result->get_error_code() ) );
			}
