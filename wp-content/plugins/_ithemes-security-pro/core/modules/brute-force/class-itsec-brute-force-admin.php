...
class ITSEC_Brute_Force_Admin {
	...
	public function itsec_add_admin_meta_boxes() {
		...
		$title = __( 'Brute Force Protection', 'it-l10n-ithemes-security-pro' );
	...
	public function itsec_admin_init() {
		...
		add_settings_section(
			'brute_force-enabled',
			__( 'Enable Brute Force Protection', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_section(
			...
			__( 'Brute Force Protection Settings', 'it-l10n-ithemes-security-pro' ),
			'__return_empty_string',
			'security_page_toplevel_page_itsec_settings'
		);
		...
		add_settings_field(
			...
			__( 'Enable local brute force protection', 'it-l10n-ithemes-security-pro' ),
	...
	public function settings_field_auto_ban_admin() {
		...
		if ( ! username_exists( 'admin' ) ) {
			...
			echo '<label for="itsec_brute_force_auto_ban_admin"> ' . __( 'Immediately ban a host that attempts to login using the "admin" username.', 'it-l10n-ithemes-security-pro' ) . '</label>';

		} else {

			echo '<p>' . __( 'You are still using an account with the username "admin." Please rename it before using this feature', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function settings_field_check_period() {
		...
		echo '<label for="itsec_brute_force_check_period"> ' . __( 'Minutes', 'it-l10n-ithemes-security-pro' ) . '</label>';
		echo '<p class="description"> ' . __( 'The number of minutes in which bad logins should be remembered.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function settings_field_enabled() {
		...
		echo '<label for="itsec_brute_force_enabled"> ' . __( 'Enable local brute force protection.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function settings_field_max_attempts_host() {
		...
		echo '<label for="itsec_brute_force_max_attempts_host"> ' . __( 'Attempts', 'it-l10n-ithemes-security-pro' ) . '</label>';
		echo '<p class="description"> ' . __( 'The number of login attempts a user has before their host or computer is locked out of the system. Set to 0 to record bad login attempts without locking out the host.', 'it-l10n-ithemes-security-pro' ) . '</p>';
