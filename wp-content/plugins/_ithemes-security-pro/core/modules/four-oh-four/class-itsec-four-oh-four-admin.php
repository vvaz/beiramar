...
class ITSEC_Four_Oh_Four_Admin {
	...
	public function check_period() {
		...
		$content .= '<label for="itsec_four_oh_four_check_period"> ' . __( 'Minutes', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function enabled() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content = sprintf( '<p class="noPermalinks">%s <a href="%soptions-permalink.php">%s</a> %s</p>',
			                    __( 'You must turn on', 'it-l10n-ithemes-security-pro' ), $adminurl, __( 'WordPress permalinks', 'it-l10n-ithemes-security-pro' ),
			                    __( 'to use this feature.', 'it-l10n-ithemes-security-pro' ) );
			...
			$content = '<input type="checkbox" id="itsec_four_oh_four_enabled" name="itsec_four_oh_four[enabled]" value="1" ' . checked( 1,
			                                                                                                                             $enabled,
			                                                                                                                             false ) . '/>';
			$content .= '<label for="itsec_four_oh_four_enabled"> ' . __( 'Enable 404 detection', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function error_threshold() {
		...
		$content .= '<label for="itsec_four_oh_four_error_threshold"> ' . __( 'Errors', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function white_list() {
		...
		$content .= '<p class="description">' . __( 'Use the white list above to prevent recording common 404 errors. If you know a common file on your site is missing and you do not want it to count towards a lockout record it here. You must list the full path beginning with the "/"', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function initialize_admin() {
		...
		add_settings_section(
			...
			__( 'Enable 404 Detection', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_section(
			...
			__( '404 Detection Settings', 'it-l10n-ithemes-security-pro' ),
			'__return_empty_string',
			'security_page_toplevel_page_itsec_settings'
		);
		...
		add_settings_field(
			...
			__( '404 Detection', 'it-l10n-ithemes-security-pro' ),
			array( $this, 'enabled' ),
			'security_page_toplevel_page_itsec_settings',
			'four_oh_four-enabled'
		);
		...
		add_settings_field(
			...
			__( 'Minutes to Remember 404 Error (Check Period)', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( 'Error Threshold', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( '404 File/Folder White List', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( 'Ignored File Types', 'it-l10n-ithemes-security-pro' ),
			array( $this, 'types' ),
			'security_page_toplevel_page_itsec_settings',
			'four_oh_four-settings'
		);
	...
	public function types() {
		...
		echo '<label for="itsec_four_oh_four_types"> ' . __( 'File types listed here will be recorded as 404 errors but will not lead to lockouts.', 'it-l10n-ithemes-security-pro' ) . '</label>';
