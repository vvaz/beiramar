...
class ITSEC_Lockout {
								...
								<?php foreach ( $host_locks as $host ) { ?>
									<li style="list-style: none;"><input type="checkbox"
										...
										<label
											...
											- <?php _e( 'Expires in', 'it-l10n-ithemes-security-pro' ); ?>
	...
	public function self_protect_metabox() {
		...
		if ( $temp !== false ) {
			...
			echo __( 'Your IP Address', 'it-l10n-ithemes-security-pro' ) . ', <strong>' . $temp['ip'] . '</strong>, ' . __( 'is whitelisted for', 'it-l10n-ithemes-security-pro' ) . ' <strong>' . human_time_diff( $itsec_globals['current_time'], $temp['exp'] ) . '</strong>.<br />';
