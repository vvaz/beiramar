...
class Two_Factor_Totp extends Two_Factor_Provider {
	...
	public function get_label() {
		return _x( 'Time-Based One-Time Password (TOTP)', 'Provider Label', 'it-l10n-ithemes-security-pro' );
	}
	...
	public function user_two_factor_options( $user ) {
		...
		$this->admin_notices();
		?>
		<p><?php esc_html_e( 'Authentication codes are generated by an app on your mobile device.', 'it-l10n-ithemes-security-pro' ); ?></p>
		<a href="javascript:;" onclick="jQuery('#two-factor-totp-options').toggle();"><?php esc_html_e( 'View Time-Based One-Time Password Configuration Details &rarr;', 'it-l10n-ithemes-security-pro' ); ?></a>
		<div id="two-factor-totp-options" style="display:none;">
			...
			<p><?php printf( _x( 'Secret: %s', 'display time-based one-time password secret', 'it-l10n-ithemes-security-pro' ), '<code><strong id="two-factor-totp-key-text">' . esc_html( $key ) . '</strong></code>' ); ?></p>
	...
	public function get_google_qr_code( $name, $key, $title = null ) {
		// Pre-encode spaces and pipes because iOS chokes on them
		$search_replace = array(
			' ' => '%20',
			'|' => '%7C',
		);
		$name = str_replace( array_keys( $search_replace ), $search_replace, $name );
